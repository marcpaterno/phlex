...
TEST_CASE("Serialize functions based on resource", "[multithreading]")
{
  ...
  auto receiving_node_for = [](tbb::flow::graph& g, std::string const& label) -> flow::function_node<unsigned int, unsigned int> {
    return flow::function_node<unsigned int, unsigned int>{
      g, flow::unlimited, [&label](unsigned int const i) -> unsigned int {
...
TEST_CASE("Serialize functions in unfold/merge graph", "[multithreading]")
{
  ...
  auto serial_node_for = [&root_resource, &root_counter](auto& g, int label) -> auto {
